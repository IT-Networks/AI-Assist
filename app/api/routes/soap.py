"""
SOAP Test-Tool v2 API Routes (Multi-Institut).

Routes:
  GET    /api/soap/config               - Konfiguration abrufen
  PUT    /api/soap/config               - Endpoints konfigurieren

  GET    /api/soap/institute            - Institute auflisten
  POST   /api/soap/institute            - Institut hinzufügen
  PUT    /api/soap/institute/{nr}       - Institut aktualisieren
  DELETE /api/soap/institute/{nr}       - Institut löschen

  GET    /api/soap/services             - Services auflisten
  POST   /api/soap/services             - Service hinzufügen
  GET    /api/soap/services/{id}        - Service-Details
  PUT    /api/soap/services/{id}        - Service aktualisieren
  DELETE /api/soap/services/{id}        - Service löschen

  POST   /api/soap/services/{id}/operations           - Operation hinzufügen
  PUT    /api/soap/services/{id}/operations/{op_id}   - Operation aktualisieren
  DELETE /api/soap/services/{id}/operations/{op_id}   - Operation löschen

  GET    /api/soap/templates/{svc}/{op}     - Template abrufen
  PUT    /api/soap/templates/{svc}/{op}     - Template speichern
  POST   /api/soap/templates/validate       - Template validieren

  POST   /api/soap/execute/{svc}/{op}       - Operation ausführen

  GET    /api/soap/session/{institut}       - Session-Status
  POST   /api/soap/session/{institut}/login - Manuelles Login
  DELETE /api/soap/session/{institut}       - Session löschen
  GET    /api/soap/sessions                 - Alle Sessions
"""
...
async def get_config() -> Dict[str, Any]:
    """Gibt die SOAP-Tool Konfiguration zurück."""
    return {
        "enabled": settings.soap_tool.enabled,
        "service_url": settings.soap_tool.service_url,
        "login_url": settings.soap_tool.login_url,
        "verify_ssl": settings.soap_tool.verify_ssl,
        "templates_path": settings.soap_tool.templates_path,
        "institut_count": len(settings.soap_tool.institute),
        "service_count": len(settings.soap_tool.services),
    }
...
async def update_config(req: ConfigRequest) -> Dict[str, Any]:
    ...
    settings.soap_tool.service_url = req.service_url
    settings.soap_tool.login_url = req.login_url
    settings.soap_tool.verify_ssl = req.verify_ssl
...
async def add_institut(req: InstitutRequest) -> Dict[str, Any]:
    ...
    existing = [i.institut_nr for i in settings.soap_tool.institute]
    ...
    settings.soap_tool.institute.append(institut)
...
async def delete_institut(institut_nr: str) -> Dict[str, Any]:
    """Löscht ein Institut."""
    before = len(settings.soap_tool.institute)
    settings.soap_tool.institute = [
        i for i in settings.soap_tool.institute if i.institut_nr != institut_nr
    ]

    if len(settings.soap_tool.institute) == before:
        raise HTTPException(status_code=404, detail=f"Institut {institut_nr} nicht gefunden")
...
async def create_service(req: ServiceRequest) -> Dict[str, Any]:
    ...
    settings.soap_tool.services.append(service)
...
async def update_service(service_id: str, req: ServiceRequest) -> Dict[str, Any]:
    ...
    for i, svc in enumerate(settings.soap_tool.services):
        if svc.id == service_id:
            ...
            settings.soap_tool.services[i] = SoapService(
                id=service_id,
                name=req.name,
                description=req.description,
                namespace=req.namespace,
...
async def delete_service(service_id: str) -> Dict[str, Any]:
    ...
    before = len(settings.soap_tool.services)
    settings.soap_tool.services = [s for s in settings.soap_tool.services if s.id != service_id]

    if len(settings.soap_tool.services) == before:
        raise HTTPException(status_code=404, detail=f"Service '{service_id}' nicht gefunden")
...
async def execute_operation(service_id: str, operation_id: str, req: ExecuteRequest) -> Dict[str, Any]:
    ...
    # Institut prüfen
    institut = next(
        (i for i in settings.soap_tool.institute if i.institut_nr == req.institut_nr and i.enabled),
        None
    )
    if not institut:
